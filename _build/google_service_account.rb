...
# Safely parse SHEETS env var
SHEETS = begin
  raw = ENV.fetch('SHEETS', nil)
  if raw.nil? || raw.strip.empty?
    []
  elsif raw.strip.start_with?('[') # JSON/Ruby array style
    # Normalize Ruby-style array to valid JSON
    normalized = raw.gsub("'", '"') # replace quates with double quoest
    JSON.parse(normalized)
  else
    raw.split(',').map(&:strip) # comma-separated
  end
end
...
def authorize_google_sheets(path, json_string)
  cred_io = if json_string && !json_string.empty?
              Jekyll.logger.info 'Using service account'
              StringIO.new(json_string)
            elsif path && File.exist?(path)
              Jekyll.logger.info 'Developing locally'
              File.open(path)
            else
...
SHEETS.each do |sheet|
  ...
  if %w[collaborators collaborations].include?(sheet.to_s.downcase)
